...
header('location:index.php');
    ...
    if(isset($_GET['pid']) && isset($_GET['action']) && $_GET['action']='restore')
    {
    $postid=intval($_GET['pid']);
    $query=mysqli_query($con,"update tblposts set Is_Active=1 where id='$postid'");
    if($query)
    {
    $msg="Post has been restored ";
    }
    else{
...
if(isset($_GET['presid']))
{
    ...
    $delmsg="delete the post completely";
}
...
<?php if(isset($delmsg)){ ?>
<div class="alert alert-success" role="alert">
<strong>succefully</strong> <?php echo htmlentities($delmsg);?></div>
<?php } ?>
...
<td>
<a href="deleteposts.php?pid=<?php echo htmlentities($row['postid']);?>&&action=restore" onclick="return confirm('Do you really want to restore ?')"> <i class="ion-arrow-return-right" title="Restore this Post"></i></a>
    &nbsp;
    <a href="deleteposts.php?presid=<?php echo htmlentities($row['postid']);?>&&action=perdel" onclick="return confirm('Do you really want to delete ?')"><i class="fa fa-trash-o" style="color: #f05050" title="Permanently delete this post"></i></a> 
 </td>
 </tr>
<?php } }?>
